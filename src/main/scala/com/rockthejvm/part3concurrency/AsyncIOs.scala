...
object AsyncIOs extends IOApp.Simple {
  ...
  def demoAsyncCancellation() = {
    val asyncMeaningOfLifeIO_v2: IO[Int] = IO.async { (cb: Callback[Int]) =>
      ...
      }.as(Some(IO("Cancelled!").debugs.void)) // IO[Unit]
    }
    for {
      fib <- asyncMeaningOfLifeIO_v2.start
      _   <- IO.sleep(500.millis) >> IO("cancelling...").debugs >> fib.cancel
      _   <- fib.join
    } yield ()
  }

  override def run: IO[Unit] = demoAsyncCancellation().debugs >> IO(threadPool.shutdown())
}
