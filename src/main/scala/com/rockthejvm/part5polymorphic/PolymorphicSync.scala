...
object PolymorphicSync extends IOApp.Simple {
  ...
  val aBlockingIO = IO.blocking { // on some specific thread pool for blocking computation
    println("loading...")
    Thread.sleep(1000)
    42
  }
  ...
  object Console {
    def make[F[_]](using sync: Sync[F]): F[Console[F]] = sync.pure((System.in, System.out)).map { case (in, out) =>
      new Console[F] {
